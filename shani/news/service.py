...
class NewsService:
    ...
    def fetch(
        ...
        *,
        ...
        # Reuse any classification we already paid for.
        for item in deduped:
    ...
    def _digest(self, items: list[NewsItem]) -> Digest:
        """One directional read for the whole feed.
        ...
        """
        rated = [i for i in items if i.lean is not Lean.UNRATED]
        bullish = sum(1 for i in rated if i.lean.score > 0)
        bearish = sum(1 for i in rated if i.lean.score < 0)
        neutral = sum(1 for i in rated if i.lean.score == 0)
        unrated = len(items) - len(rated)

        if not rated:
            return Digest(
                lean=Lean.UNRATED, score=0.0, bullish=0, bearish=0,
                neutral=0, unrated=unrated,
                headline="Nothing classified yet — configure a model to read the tape.",
            )
        ...
        for item in rated:
            ...
            weighted += item.lean.score * weight
            total_weight += weight
        ...
        if lean is Lean.NEUTRAL:
            headline = f"Tape reads balanced — {bullish} up, {bearish} down, {neutral} neutral."
        else:
            direction = "higher" if score > 0 else "lower"
            headline = (
                f"News leans {direction}: {bullish} bullish vs {bearish} bearish "
                f"across {len(rated)} classified items."
            )
