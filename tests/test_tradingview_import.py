...
symbol. An account holding instruments at very different price levels then never
closes a position against its own instrument, and the result was round trips
wrong by orders of magnitude — a six-figure phantom loss on an account that was
up four figures. It threw no exception and every other test stayed green.

**Double counting.** An import re-reads the whole history, so importing twice
must leave the same number of trades, not twice as many. That one is in
``tests/test_integration.py``, where the seam tests live.

Every fixture here is synthetic. No real fills, balances or account identifiers
...
def test_simple_long_round_trip() -> None:
    trips = pair_executions([ex(0, 1, 10, "7664"), ex(60, -1, 10, "7672.25")])
    assert len(trips) == 1
    trip = trips[0]
    assert trip.side is Side.BUY
    assert trip.quantity == 10
    assert trip.entry_price == Decimal("7664")
    assert trip.exit_price == Decimal("7672.25")
...
def test_short_round_trip_keeps_its_direction() -> None:
    ...
    trips = pair_executions([ex(0, -1, 10, "7707"), ex(60, 1, 10, "7701.75")])
    assert trips[0].side is Side.SELL
    assert trips[0].entry_price == Decimal("7707")
    assert trips[0].exit_price == Decimal("7701.75")
...
def test_scale_in_averages_entry_by_size() -> None:
    ...
    trips = pair_executions(
        [ex(0, 1, 10, "7600"), ex(30, 1, 30, "7700"), ex(60, -1, 40, "7710")]
    )
    assert len(trips) == 1
    assert trips[0].quantity == 40
    assert trips[0].entry_price == Decimal("7675")


def test_scale_out_is_one_trade_not_two() -> None:
    trips = pair_executions(
        [ex(0, 1, 10, "7600"), ex(30, -1, 4, "7610"), ex(60, -1, 6, "7620")]
    )
    assert len(trips) == 1
    assert trips[0].quantity == 10
    assert trips[0].exit_price == Decimal("7616")  # (4*7710 + 6*7720) / 10


def test_reversal_splits_into_two_trades() -> None:
    """Long 10, then sell 30: the first 10 close the long, 20 open a short."""
    trips = pair_executions(
        [ex(0, 1, 10, "7600"), ex(30, -1, 30, "7650"), ex(60, 1, 20, "7640")]
    )
    assert len(trips) == 2
    assert trips[0].side is Side.BUY
    assert trips[0].quantity == 10
    assert trips[0].exit_price == Decimal("7650")
    assert trips[1].side is Side.SELL
    assert trips[1].quantity == 20
    assert trips[1].entry_price == Decimal("7650")


def test_position_still_open_is_reported_not_dropped() -> None:
    trips = pair_executions([ex(0, 1, 10, "7600"), ex(30, -1, 4, "7610")])
    assert len(trips) == 1
...
def test_symbols_are_paired_independently() -> None:
    ...
    fills = [
        ex(0, 1, 10, "7600"),
        ex(10, -1, 5, "29614.25", symbol="CME_MINI:MNQ1!"),
        ex(20, -1, 10, "7610"),
        ex(30, 1, 5, "29437", symbol="CME_MINI:MNQ1!"),
    ]
    trips = pair_executions(fills)
    assert len(trips) == 2
    by_symbol = {t.symbol: t for t in trips}
    assert by_symbol["CME_MINI:MESU2026"].quantity == 10
    assert by_symbol["CME_MINI:MESU2026"].exit_price == Decimal("7610")
    assert by_symbol["CME_MINI:MNQ1!"].side is Side.SELL
    assert by_symbol["CME_MINI:MNQ1!"].exit_price == Decimal("29437")
...
def test_out_of_order_fills_are_sorted_before_pairing() -> None:
    ...
    trips = pair_executions([ex(60, -1, 10, "7672.25"), ex(0, 1, 10, "7664")])
    assert len(trips) == 1
    assert trips[0].side is Side.BUY
    assert trips[0].entry_price == Decimal("7664")
...
def test_dated_contract_maps_to_root_and_keeps_the_contract() -> None:
    ...
    """
    report = build_trades(
        [ex(0, 1, 10, "7664"), ex(60, -1, 10, "7672.25")], account="ACCOUNT-1"
    )
...
def test_continuous_symbol_has_no_contract() -> None:
    report = build_trades(
        [
            ex(0, 1, 5, "7630.5", symbol="CME_MINI:MES1!"),
            ex(60, -1, 5, "7673", symbol="CME_MINI:MES1!"),
        ]
    )
...
def test_unknown_instruments_are_skipped_and_reported() -> None:
    ...
    report = build_trades(
        [
            ex(0, 1, 10, "7664"),
            ex(30, 1, 10, "664.89", symbol="AMEX:SPY"),
            ex(60, -1, 10, "7672.25"),
            ex(90, -1, 10, "665.42", symbol="AMEX:SPY"),
        ]
    )
    assert report.count == 1
...
def test_paper_commission_is_absent_not_invented() -> None:
    ...
    """
    report = build_trades([ex(0, 1, 10, "7664"), ex(60, -1, 10, "7672.25")])
...
def test_bracket_supplies_planned_risk_and_r_multiple() -> None:
    ...
    orders = [
        ...
        TradingViewOrder(
            ...
            order_type="stop", status="cancelled", stop_price=Decimal("7660.25"),
            placed_at=entry_at - timedelta(seconds=300),
        ),
    ]
    report = build_trades([ex(0, 1, 10, "7664"), ex(60, -1, 10, "7672.25")], orders)
    trade = report.trades[0]
    assert trade.initial_stop == Decimal("7660.25")
...
def test_ambiguous_bracket_declines_rather_than_guessing() -> None:
    ...
    orders = [
        ...
        for i, stop in enumerate(["7660.25", "7658.5"])
    ]
    report = build_trades([ex(0, 1, 10, "7664"), ex(60, -1, 10, "7672.25")], orders)
...
def test_trade_uuid_is_stable_and_distinct() -> None:
    a = "tradingview:ACCOUNT-1:CME_MINI:MESU2026:2479343643"
    b = "tradingview:ACCOUNT-1:CME_MINI:MESU2026:2479521076"
...
def test_same_fills_produce_the_same_trade_id() -> None:
    ...
    fills = [ex(0, 1, 10, "7664"), ex(60, -1, 10, "7672.25")]
...
def test_reconciles_with_the_venues_reported_pnl() -> None:
    """Four round trips whose total must match what the venue reports.

    This is the shape of the check that validated the importer in the first
    place. It was not accepted because its tests passed, but because its total
    realized P&L reproduced, to the cent, the figure the broker showed for the
    same account. That check needs a live account and so cannot run in CI; this
    pins the arithmetic it depends on.

    If this figure drifts, the pairing is wrong and every statistic computed
    from imported trades is wrong with it.
    """
    fills = [
        ex(0, 1, 10, "7670.25"), ex(100, -1, 10, "7679.75"),    # +475.00
        ex(200, 1, 10, "7664"), ex(300, -1, 10, "7672.25"),     # +412.50
        ex(400, 1, 10, "7705"), ex(500, -1, 10, "7703.25"),     # -87.50
        ex(600, -1, 10, "7707"), ex(700, 1, 10, "7701.75"),     # +262.50
    ]
    trips = pair_executions(fills)
    assert len(trips) == 4
    assert realized_pnl(trips) == Decimal("1062.50")

    report = build_trades(fills, account="ACCOUNT-1")
